---
id: comparison
url: total/java/developer-guide/comparison
title: Compare documents with GroupDocs.Comparison
linkTitle: Comparison
weight: 3
description: "Find the differences between two versions of a document from GroupDocs.Total for Java, list every change, and accept or reject them — runnable Java examples with downloadable files."
keywords: GroupDocs.Comparison, compare documents, document diff, track changes, accept reject changes, Java
...
The examples below compare two revisions of the same contract. `contract-v1.docx` and `contract-v2.docx` differ in exactly three places: the delivery date, the contract value, and a late-delivery clause added in v2. Each example is ready to copy into a project once you have [installed the package]({{< ref "/total/java/getting-started/installation.md" >}}).
...
try (Comparer comparer = new Comparer("contract-v1.docx")) {
...
{{< tab "Sample files" >}}
{{< tab-text >}}
`contract-v1.docx` and `contract-v2.docx` are the sample files used in this example. Download [contract-v1.docx](/total/java/_sample_files/contract-v1.docx) and [contract-v2.docx](/total/java/_sample_files/contract-v2.docx).
{{< /tab-text >}}
{{< /tab >}}
...
```
[Download full output](/total/java/_output_files/developer-guide/comparison/ComparisonCompareDocuments/comparison-compare-documents.docx)
{{< /tab >}}
{{< /tabs >}}
...
try (Comparer comparer = new Comparer("contract-v1.docx")) {
    ...
    System.out.println("Changes found: " + changes.length);

    for (ChangeInfo change : changes) {
        System.out.println(change.getId() + ". " + change.getType()
...
[TRUNCATED]
```
[Download full output](/total/java/_output_files/developer-guide/comparison/ComparisonListChanges/comparison-list-changes.txt)
{{< /tab >}}
{{< /tabs >}}
...
try (Comparer comparer = new Comparer("contract-v1.docx")) {
    ...
    if (changes.length > 0) {
        ...
        System.out.println("Rejected: '" + changes[0].getTargetText() + "'");
    }

    ApplyChangeOptions applyOptions = new ApplyChangeOptions();
    applyOptions.setChanges(changes);
...
Binary file (DOCX, 16 KB)
```
[Download full output](/total/java/_output_files/developer-guide/comparison/ComparisonAcceptReject/comparison-accept-reject.docx)
{{< /tab >}}
{{< /tabs >}}
...
* [GroupDocs.Comparison for Java documentation](https://docs.groupdocs.com/comparison/java/) — the full product guide
* [Supported file formats](https://docs.groupdocs.com/comparison/java/supported-document-formats/)
